...
const StatusMapping: Record<string, string> = {
  ...
  MemberNotFound:
    'Nie znaleziono aktywnego członka w Harcerskim Serwisie Szkoleniowym.',
};

export default function VerifyCertificates() {
  const [formState, formAction] = useFormState<
    VerifyCertificatesState,
    FormData
  >(verifyCertificates, {});
          ...
          <tbody>
            {certificates?.map((certificate, index) => (
              <tr
                key={index}
                className={clsx({
                  'text-error': certificate.status !== 'Valid',
                })}
              >
                <th>{index + 1}</th>
                <td>{certificate.membershipNumber}</td>
                <td>
                  {StatusMapping[certificate.status] ??
...
function VerifyCertificatesForm({
  action,
  state,
}: {
  action: (data: FormData) => void;
  state: VerifyCertificatesState;
}) {
  return (
    <form action={action} className="flex flex-col gap-4">
      <label className="form-control">
        ...
        <textarea
          className={clsx('textarea textarea-bordered h-48', {
            'textarea-error': state?.message,
          })}
