...
export async function createAccounts(
  prevState: CreateAccountsState,
  formData: FormData,
): Promise<CreateAccountsState> {
  ...
  if (!validatedFields.success) {
    return {
      errors: Object.values(
        validatedFields.error.issues.reduce((acc, issue) => {
          update(acc, issue.path, (v) =>
            v ? [...v, issue.message] : [issue.message],
          );
          return acc;
        }, {}),
      ),
  ...
  return {
    accounts: validatedFields.data.map((account) => ({
      ...account,
      status: 'Success',
      password: 'Test123!',
    })),
...
export async function verifyCertificates(
  prevState: VerifyCertificatesState,
  formData: FormData,
): Promise<VerifyCertificatesState> {
  ...
  const invalidMembershipNumbers = membershipNumbers?.filter(
    (membershipNumber) => !MEMBERSHIP_NUMBER_REGEX.test(membershipNumber),
  );

  if (invalidMembershipNumbers?.length) {
    return {
      message:
        invalidMembershipNumbers.length > 1
